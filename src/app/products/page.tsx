...
const ProductsPage = memo(function ProductsPage() {
  ...
  const handleToggleWishlist = useCallback((product: typeof products[0], e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleBuyNow = useCallback((product: typeof products[0]) => {
    addToCart(product);
        ...
        {products.map((product) => {
          ...
          return (
            <div key={product.id} className="relative bg-white rounded-2xl shadow overflow-hidden">
              ...
              >
                <Heart size={20} fill={inWishlist ? 'red' : 'none'} strokeWidth={inWishlist ? 1 : 2} />
              </button>
